...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        # Declare the 'use_world_frame' argument
        DeclareLaunchArgument(
            'use_world_frame',
            default_value='false',  # Default to false (vehicle-relative frame)
            description='Select whether to use world frame (NED) or vehicle-relative frame (FLU)'
        ),
        ...
        # Node for the velocity_control with joystick config, if input is 'joystick'
        Node(
            ...
            ],  # Pass joystick config file and use_world_frame parameter
            condition=IfCondition(
                PythonExpression([
                    "'",
                    LaunchConfiguration('input'),
        ...
        # Node for velocity_control with keyboard config, if input is 'keyboard'
        Node(
            package='px4_ros_offboard',
            executable='velocity_control',
            name='velocity_control_keyboard',
            output='screen',
            parameters=[
                {'config': keyboard_config_file_path},
                {'use_world_frame': LaunchConfiguration('use_world_frame')}
            ],  # Pass keyboard config file and use_world_frame parameter
            condition=IfCondition(
                PythonExpression([
                    "'",
                    LaunchConfiguration('input'),
